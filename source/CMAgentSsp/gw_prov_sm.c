...
void GWP_UpdateERouterMode(void)
{
    ...
    if (oldRouterMode != eRouterMode)
    {
        ...
        GWPROV_PRINT(" %s : updating WanManager Ip mode: %s\n", __FUNCTION__, IpModeString[eRouterMode]);
           ...
           GWPROV_PRINT(" %s : name = %s, val = %s \n", __FUNCTION__, name, val);
	    ...
	    else if (ret_value == CURRENT_WAN_IPADDR)
            {
                ...
                if (!sIPv4_acquired && val )
                {
                    ...
                    ERR_CHK(rc);
                    if ((ind != 0) && (rc == EOK)){
                        v_secure_system("print_uptime \"wan_ipv4_up\"");
                        sIPv4_acquired = 1;
                    }
                }
            }
...
static void GWP_act_DocsisLinkDown_callback_1()
{
    ...
    GWPROV_PRINT("%s %d - Docsis Link Down : setting ethernet port %s down\n", __FUNCTION__, __LINE__,IFNAME_ETH_0);
...
static void * DocsisLinkMonitorThread(void *pVoid)
{
    ...
    if (NULL != pVoid)
    {
        GWPROV_PRINT("%s %d - Un Used variable \n", __FUNCTION__, __LINE__);
    }
    ...
    {
        GWPROV_PRINT("%s:%d, Failed to read CM status",__FUNCTION__,__LINE__);
        sleep(5);
        if(docsis_getCMStatus(cBuf) != RETURN_OK)
        {
            GWPROV_PRINT("%s:%d, Failed to read CM status",__FUNCTION__,__LINE__);
            GWPROV_PRINT("%s:%d, Not taking the WAN down, returning from here",__FUNCTION__,__LINE__);
    ...
    if (!strcmp(cBuf,"OPERATIONAL"))
    {
        ...
        {
            GWPROV_PRINT("%s %d - allowremoteinterfaces is set to 1, An XLE is connected\n", __FUNCTION__, __LINE__);
            iMaxCount = MAX_WAIT_WFO_ENABLED_DEVICES; //Wait for 30 seconds
        }
        else
        {
            GWPROV_PRINT("%s %d - allowremoteinterfaces is set to 0, No LTE Connected\n", __FUNCTION__, __LINE__);
        ...
        if (0 == iAllowRemoteInterfaces)
        {
            ...
            {
                GWPROV_PRINT("%s %d - CM status is Still:%s\n", __FUNCTION__, __LINE__,cBuf);
                memset(cBuf,0,sizeof(cBuf));
                sysevent_get(sysevent_fd_gs, sysevent_token_gs, "ping-status", cBuf, sizeof(cBuf));
                if (strcmp(cBuf,"missed") == 0)
                {
                    GWPROV_PRINT("%s %d - ping miss event is set, setting wan down\n", __FUNCTION__, __LINE__);
                    bIsWanStatusSetDown = true;
                    setWanStatusDown ();
                }
                else
                {
                    GWPROV_PRINT("%s %d - ping miss event is NOT set, Not setting wan down\n", __FUNCTION__, __LINE__);
                }
            }
            bThreadCreated = false;
            pthread_mutex_unlock(&gWanDownMutex);
    ...
    {
        GWPROV_PRINT("%s %d - Setting the WAN status down\n", __FUNCTION__, __LINE__);
        bIsWanStatusSetDown = true;
        setWanStatusDown ();
    }
    pthread_mutex_unlock(&gWanDownMutex);
...
static void GWP_act_DocsisLinkDown_callback_2()
{
    ...
    {
        DocsisLd_cfg.HAL_DocsisLinkdownEnable=true;
        GWPROV_PRINT("%s:%d, setting the HAL_DocsisLinkdownEnable to true\n",__FUNCTION__,__LINE__);
        return;
    }
#endif

    pthread_mutex_init(&lock, NULL); // Initialize the mutex
    GWPROV_PRINT("%s:%d, Entry, mutex added \n", __FUNCTION__, __LINE__);
    eLinkStatus = STATUS_DOWN;

    if ((true == bLinkDownSimulation) && (access("/tmp/linkDownFlagSet", F_OK) != 0))
    {
        GWPROV_PRINT("%s %d - LinkDown simulation flag is set\n", __FUNCTION__, __LINE__);
        setWanStatusDown ();
    ...
    else
    {
        ...
        if (false == bThreadCreated)
        {
            ...
            {
                GWPROV_PRINT("%s %d - DocsisLinkMonitorThread created\n", __FUNCTION__, __LINE__);
                bThreadCreated = true;
            }
        }
...
void* UpdateOperStatusThread( void *arg)
{
    ...
    while(1)
    {
        ...
        {
            GWPROV_PRINT("%s %d getCMStatus %s \n", __FUNCTION__, __LINE__,buf);
...
static int GWP_act_DocsisLinkUp_callback()
{
    ...
    if ((true == bThreadCreated) && (false == bIsWanStatusSetDown))
    {
        if (!strcmp(cBuf,"OPERATIONAL"))
        {
            GWPROV_PRINT("%s %d - CM status is not down, it is Operational, So ignoring the Callback UP Event\n", __FUNCTION__, __LINE__);
        }
        pthread_mutex_unlock(&gWanDownMutex);
        return 0;
    }
    ...
    if (ret == 0)
    {
       GWPROV_PRINT("%s %d Two Box LAN Port:Re-Initializing:WAN\n", __FUNCTION__, __LINE__);
       GWPROV_PRINT("%s %d Docsis Link Up : setting ethernet port %s UP\n", __FUNCTION__, __LINE__, macsec_ifname);
       v_secure_system("ip link set dev %s up",macsec_ifname);
    }
    else
       GWPROV_PRINT("%s %d Docsis Link Up : ethernet port %s is already UP\n", __FUNCTION__, __LINE__, macsec_ifname);
...
static int GWP_act_DocsisCfgfile_callback(char *cfgFile)
{
    ...
    else if ((unsigned int)actualNumBytes != cfgFileBuffLen)
    {
        	...
        	GWPROV_PRINT(" eSafe Config file \"%s\", actual len (%lu) different than stat (%d), aborting Config file\n", cfgFileName, actualNumBytes, cfgFileBuffLen);
        #else
...
if( uid == 0 )
{
    ...
    GWPROV_PRINT(" %s sysevent open fd %d fd_gs %d  \n",__FUNCTION__,sysevent_fd,sysevent_fd_gs);
    if (sysevent_fd >= 0)
    {
...
int GWP_PushEventInMsgq(ClbkInfo *pInfo )
{
    ...
    if (!pInfo)
        return -1;
    GWPROV_PRINT("%s:%d: before open \n", __FUNCTION__, __LINE__);
    mq = mq_open(EVENT_QUEUE_NAME, O_WRONLY | O_NONBLOCK);
    if (!((mqd_t)-1 != mq)) 
    {
    GWPROV_PRINT("%s:%d: mq open failed\n", __FUNCTION__, __LINE__);
        return -1;
    }
    ...
    GWPROV_PRINT("%s:%d: before send \n", __FUNCTION__, __LINE__);
    ret = mq_send(mq, buffer, MAX_SIZE, 0);
    if (ret != 0)
    {
        GWPROV_PRINT("%s:%d: mq send failed \n", __FUNCTION__, __LINE__);
    }
    if (-1 != mq_close(mq))
    {
        GWPROV_PRINT("%s:%d: mq close failed \n", __FUNCTION__, __LINE__);
    }
    GWPROV_PRINT("%s:%d: exited \n", __FUNCTION__, __LINE__);
    return 0;
}
...
void *GWP_EventHandler(void *arg)
{
    ...
    do
    {
        ...
        GWPROV_PRINT("%s:%d: check wanmanger is ready \n", __FUNCTION__, __LINE__);
        while( 0 != access( "/tmp/wanmanager_initialized" , F_OK ) )
        {
            GWPROV_PRINT("%s:%d: waiting till component comes up \n", __FUNCTION__, __LINE__);
            sleep(1);
        }
        ...
        switch (EventMsg.EventType)
        {
            case EVENT_GWP_LINK_UP:
            {
                ...
                if(!g_OperStatusThread_Running)
                {
                    ...
                    {
                        GWPROV_PRINT("%s %d - UpdateOperStatusThread Successfully\n", __FUNCTION__, __LINE__);
                        snprintf(paramValue,sizeof(paramValue),"Up");
                    }
                }else
                {
                    GWPROV_PRINT("%s %d - UpdateOperStatusThread Already running\n", __FUNCTION__, __LINE__);
                    snprintf(paramValue,sizeof(paramValue),"Up");
                }
                continue;
...
int RemoveIfFileExists(const char *fname)
{
	...
	if(ret == 0) 
	{
            GWPROV_PRINT("Removed file %s \n",fname);
        } 
        else 
	{ 
	    GWPROV_PRINT("Failed to remove file %s \n",fname);
        }
        close(fd);
        return 0;
}
...
void CreateThreadandSendCondSignalToPthread()
{
	...
	if(cmAgent_Link_Status.DocsisLinkDown==true)
	{
		...
		if(status)
		{
			GWPROV_PRINT("WAN_Failover_Simulation pthread create fail \n");
			return;
		}
		else
		{
			GWPROV_PRINT(" WAN_Failover_Simulation Pthread is created\n");
		}
	}
	else if(cmAgent_Link_Status.DocsisLinkDown==false)
	{
		pthread_cond_signal(&LinkdownCond);
		GWPROV_PRINT(" pthread_cond_signal CmAgent_Link.DocsisLinkDown false \n");
	}
}
/************************************************************************************
...
void CreateDocsisLinkDown_TestFile()
{
	...
	if(LinkdownPtr==NULL)
	{
		GWPROV_PRINT("%s file creates fail\n",DOCSISLINKDOWN_TESTFILE);
		return;
	}
	fclose(LinkdownPtr);
}
...
{
	pthread_detach(pthread_self());
	GWPROV_PRINT(" Enter into %s thread!!\n",__FUNCTION__);
	...
	else
	{
		...
		if(rc)
	        {
		    GWPROV_PRINT(" pthread_cond_timedwait Error in %s \n", __FUNCTION__);
		}
...
void RegisterDocsisCallback()
{
...
#endif
    GWPROV_PRINT("Entry docsis clbk register!!\n");
#if !defined(INTEL_PUMA7)
    ...
    GWPROV_PRINT(" create Gwp callback event handler\n");
